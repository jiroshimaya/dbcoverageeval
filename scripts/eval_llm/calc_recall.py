...
def compute_recall_at_k(
    ...
    # Recall@Kを計算
    total_recall = 0
    for i, (top_indices, pos_indices) in enumerate(
        zip(top_k_indices, positive_indices)
    ):
...
def main():
    ...
    for query in queries:
        ...
        pos_indices = [
            i for i, docid in enumerate(corpus_docids) if docid in pos_docids
        ]
        if not pos_indices:
    ...
    recall = compute_recall_at_k(
        query_embeddings, corpus_embeddings, positive_indices, k=TOP_K
    )
